...
class TransformerDataGenerator:
    ...
    def get_game_info(self):
        ...
        query = """
            ...
            AND (
                to_timestamp(gr.gamedatetimeest, 'YYYY-MM-DD HH24:MI:SS') BETWEEN '2023-10-24 00:00:00' AND '2024-04-14 23:59:59'
                OR 
                to_timestamp(gr.gamedatetimeest, 'YYYY-MM-DD HH24:MI:SS') BETWEEN '2024-10-22 00:00:00' AND '2025-04-13 23:59:59'
            )
        ...
        teammate_matrix.columns = [
            f"tm{int(col[1])}_{col[0]}" for col in teammate_matrix.columns
        ]
        ...
        backup_pivot.columns = [f"tm5_{col[0]}" for col in backup_pivot.columns]
        ...
        for i in range(1, 5):  # Loop through slots 1, 2, 3, 4
            ...
            for stat in TEAMMATE_STAT_COLS:
                ...
                df[f"tm{i}_{stat}"] = np.where(
                    is_self, df[f"tm5_{col[0]}"], df[f"tm{i}_{stat}"]
                )
        ...
        team_profile["blended_std_ortg"] = np.where(
            ...
            + (weight) * team_profile["std_ortg_raw"].fillna(team_profile["prior_ortg"]),
        ...
        team_profile["blended_std_drtg"] = np.where(
            ...
            + (weight) * team_profile["std_drtg_raw"].fillna(team_profile["prior_drtg"]),
        ...
        opp_ortg_lookup = team_profile[
            ["gameid", "playerteamname", "std_ortg_raw"]
        ].rename(
            columns={
                "playerteamname": "opponentteamname",
        ...
        # 1. Merge Team Stats to your main player dataframe
...
# def _get_season(self, game_date) -> int:
